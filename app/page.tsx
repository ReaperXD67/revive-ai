  ...
  return (
    <main className="app-shell">
      ...
      {demoOpen && <DemoModal
        ...
        error={demoError}
        onStart={startDemo}
        onInspectProof={() => { setDemoOpen(false); setRunning(false); chooseView('proof'); }}
        onClose={() => { setDemoOpen(false); setRunning(false); }}
      />}
...
function DemoModal({ step, running, result, error, onStart, onInspectProof, onClose }: { step: number; running: boolean; result: SimulationResponse | null; error: string; onStart: () => void; onInspectProof: () => void; onClose: () => void }) {
  ...
  return <div className="overlay modal-overlay" role="dialog" aria-modal="true" aria-label="Live recovery demo"><button className="overlay-dismiss" onClick={onClose} aria-label="Close live demo" /><section className="demo-modal"><div className="drawer-header"><div><p className="eyebrow">INTERACTIVE RECOVERY SIMULATION</p><h2>Watch ₹11,999 come back.</h2></div><button autoFocus onClick={onClose} aria-label="Close"><X size={18} /></button></div><p className="demo-intro">A realistic Razorpay <code>subscription.pending</code> event enters the hosted decision API. Every response is runtime-validated, policy-checked, idempotent and stored in a durable audit trail.</p><div className="incoming-event"><span className="event-icon"><RadioTower size={17} /></span><div><small>INCOMING WEBHOOK</small><strong>Nisha Menon · Pro annual</strong><span>Insufficient balance · UPI AutoPay</span></div><strong>₹11,999</strong></div><div className="demo-steps" role="status" aria-live="polite" aria-busy={running}>{steps.map(({ icon: Icon, title, detail }, index) => { const complete = step > index; const active = running && step === index; return <div className={`demo-step ${complete ? 'complete' : ''} ${active ? 'active' : ''}`} key={title}><span>{complete ? <Check size={16} /> : <Icon size={16} />}</span><p><strong>{title}</strong><small>{detail}</small></p><i>{complete ? 'Done' : active ? 'Working…' : 'Waiting'}</i></div>; })}</div>{step >= 4 && result ? <><div className="demo-success" role="status" aria-live="polite"><div><CheckCircle2 size={24} /></div><p><small>SIMULATED PAYMENT CAPTURED</small><strong>₹11,999 recovered</strong><span>{result.plan.action.replaceAll('_', ' ')} · {Math.round(result.plan.confidence * 100)}% confidence · {result.plan.executionMode}</span></p></div><div className="demo-proof"><span><small>POLICY</small><strong>v{result.policyVersion}</strong></span><span><small>AUDIT STORE</small><strong>{result.persistence.replaceAll('_', ' ')}</strong></span><span><small>REQUEST PROOF</small><strong>{result.requestId.slice(0, 8)}…</strong></span></div><div className="demo-success-actions"><button className="primary-dark" onClick={onInspectProof}><Fingerprint size={14} /> Inspect audit proof <ArrowRight size={14} /></button></div></> : <div className="demo-controls"><div><ShieldCheck size={15} /><span>Simulation mode · no real payment or message will be sent</span></div><button className="primary-dark" onClick={onStart} disabled={running}><Play size={14} fill="currentColor" />{running ? 'Agents working…' : step ? 'Restart simulation' : 'Start simulation'}</button></div>}{error ? <div className="demo-error" role="alert"><CircleAlert size={15} /><span>{error} No recovery was claimed.</span></div> : null}<details className="demo-research"><summary>Why this simulation is realistic <ChevronDown size={14} /></summary>{researchFacts.map((fact) => <p key={fact}><Check size={13} />{fact}</p>)}</details></section></div>;
}
