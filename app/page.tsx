  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      if (demoStep >= 3) {
        setDemoStep(4);
        setRunning(false);
        setToast('₹24,999 recovered. The subscription is active again.');
      } else {
  ...
  const startDemo = async () => {
    ...
    try {
      await fetch('/api/recovery/simulate', {
        ...
        body: JSON.stringify({
          eventId: 'evt_demo_4821', customerId: 'cust_demo_nisha', amount: 24999,
...
function DemoModal({ step, running, onStart, onClose }: { step: number; running: boolean; onStart: () => void; onClose: () => void }) {
  ...
  return <div className="overlay modal-overlay" role="dialog" aria-modal="true" aria-label="Live recovery demo"><button className="overlay-dismiss" onClick={onClose} aria-label="Close live demo" /><section className="demo-modal"><div className="drawer-header"><div><p className="eyebrow">INTERACTIVE RECOVERY SIMULATION</p><h2>Watch ₹24,999 come back.</h2></div><button onClick={onClose} aria-label="Close"><X size={18} /></button></div><p className="demo-intro">A realistic Razorpay <code>subscription.pending</code> event enters Revive. Every decision is explainable, policy-checked and duplicate-safe.</p><div className="incoming-event"><span className="event-icon"><RadioTower size={17} /></span><div><small>INCOMING WEBHOOK</small><strong>Nisha Menon · Pro annual</strong><span>Insufficient balance · UPI AutoPay</span></div><strong>₹24,999</strong></div><div className="demo-steps">{steps.map(({ icon: Icon, title, detail }, index) => { const complete = step > index; const active = running && step === index; return <div className={`demo-step ${complete ? 'complete' : ''} ${active ? 'active' : ''}`} key={title}><span>{complete ? <Check size={16} /> : <Icon size={16} />}</span><p><strong>{title}</strong><small>{detail}</small></p><i>{complete ? 'Done' : active ? 'Working…' : 'Waiting'}</i></div>; })}</div>{step >= 4 ? <div className="demo-success"><div><CheckCircle2 size={24} /></div><p><small>PAYMENT CAPTURED</small><strong>₹24,999 recovered</strong><span>Subscription restored · customer retained · event logged</span></p></div> : <div className="demo-controls"><div><ShieldCheck size={15} /><span>Simulation mode · no real payment or message will be sent</span></div><button className="primary-dark" onClick={onStart} disabled={running}><Play size={14} fill="currentColor" />{running ? 'Agents working…' : step ? 'Restart simulation' : 'Start simulation'}</button></div>}<details className="demo-research"><summary>Why this simulation is realistic <ChevronDown size={14} /></summary>{researchFacts.map((fact) => <p key={fact}><Check size={13} />{fact}</p>)}</details></section></div>;
}
